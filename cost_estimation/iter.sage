...
def cost_of_proof(T, gamma,kappa, parameters):
    return (parameters['mult_square_ratio']*(T / kappa + gamma * (2**kappa))).ceil();
...
def cost_of_proof_for_hybrid(T, N, gamma,kappa, parameters):
    t = (T/2**N).n().ceil();
    return (parameters['mult_square_ratio']*(t / kappa + gamma * (2**kappa) \
        + 1.5* parameters['security_level']*2**N*t/(kappa*gamma) )).ceil();
...
def test(T, parameters):
    ...
    for N in range(1,8):
        ...
        print "Overhead: ", (100*optimize_hybrid(T,N-1, parameters)/T).n(5), "%\n";

    for N in range(1,8):
        print "%d-iterated prover" % N;
        print "Size of proofs: ", (N*parameters['modulus_length'] \
                                  + (N-1)*2*parameters['security_level'])/8, "B";
        print "Overhead: ", (100*optimize_iterated(T,N, parameters)[1]/T).n(5), "%\n";
